<template>
    <div class="watch-player" :class="{
        ...
        'watch-player--pseudo-fullscreen': playerStore.is_pseudo_fullscreen,
    }">
...
.watch-player {
    ...
    // 縦画面向けの aspect-ratio 固定を打ち消して映像を全面表示する
    // (物理的に横画面になっている場合は元々 aspect-ratio 指定がないため対象外)
    &.watch-player--pseudo-fullscreen {
        @media (orientation: portrait) {
            aspect-ratio: auto;

            // 擬似フルスクリーン表示時はパネルの開閉自体が可能になるため、
            // 縦画面向けに非表示にしていたパネル開閉ボタンを再表示する
            .watch-player__button .switch-button-panel {
                display: flex;
            }

            // 見た目上は横画面表示になるため、ボタン間隔も smartphone-horizontal と同じ間隔にする
            // (縦画面向けの smartphone-vertical の値のままだとボタン間隔が狭くなってしまう)
